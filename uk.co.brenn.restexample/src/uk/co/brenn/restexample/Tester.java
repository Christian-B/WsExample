...
public class Tester {
  public static void main(String[] args) {
    ...
    // Create one todo
    Todo todo = new Todo("1", "Blabla");
    ClientResponse response = service.path("todos")
        .path(todo.getId()).accept(MediaType.APPLICATION_XML)
        .put(ClientResponse.class, todo);
    // Return code should be 201 == created resource
    System.out.println(response.getStatus());
    // Get the Todos
    System.out.println(service.path("todos")
    ...
    System.out.println(service.path("todos/1")
        .accept(MediaType.APPLICATION_XML).get(String.class));
    // get Todo with id 1
    service.path("todos/1").delete();
